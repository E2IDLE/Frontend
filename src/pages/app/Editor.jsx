...
export default function AppShell({ setPage }) {
  ...
  // 2. 에이전트 상태 폴링 및 서버 등록 (Registration)
  useEffect(() => {
  ...
  const acceptNotification = async (id, name, sessionId, peerId, multiAddress, t, inviteCode) => {
    ...
    // 최종 타겟 정보를 에이전트에게 전달 (홀펀칭 명령)
    if (targetPeerId && targetMultiAddress) {
      try {
        toast("에이전트 연결 시도 중...", "ok");
        ...
        if (res.ok) {
          ...
          toast(`${name}님과 연결되었습니다!`, "ok");
        } else {
          const json = await res.json().catch(() => ({}));
          toast(`연결 에러: ${json.errorCode || "에이전트 거부"}`, "err");
        }
      } catch {
        toast("에이전트에 연결할 수 없습니다.", "err");
      }
    } else {
      toast("상대방 주소 정보가 부족합니다.", "err");
    }
